 ...
 */
class Sminmlc_TargetRule_Model_Resource_Index extends Mage_Index_Model_Resource_Abstract
{
    /**
    ...
    /**
     * Constructor for Sminmlc_TargetRule_Model_Resource_Index
     *
     * @param array $args
     */
    ...
    {
        $this->_init('sminmlc_targetrule/index', 'entity_id');
    }
     ...
     *
     * @param int $type
     * @return Sminmlc_TargetRule_Model_Resource_Index_Abstract
     */
    public function getTypeIndex($type)
    {
        switch ($type) {
            ...
            default:
                Mage::throwException(
                    Mage::helper('sminmlc_targetrule')->__('Undefined Catalog Product List Type')
                );
        }

        return Mage::getResourceSingleton('sminmlc_targetrule/index_' . $model);
    }
    ...
    {
        return array(
            Sminmlc_TargetRule_Model_Rule::RELATED_PRODUCTS,
            Sminmlc_TargetRule_Model_Rule::UP_SELLS,
            Sminmlc_TargetRule_Model_Rule::CROSS_SELLS
        );
    }
    ...
    public function getProductIds($object)
    {
        ...
        $rotationMode = $this->_factory->getHelper('sminmlc_targetrule')->getRotationMode($object->getType());
        ...
        if ($rotationMode == Sminmlc_TargetRule_Model_Rule::ROTATION_SHUFFLE) {
             shuffle($productIds);
        }
        return $productIds;
    }
    ...
    protected function _matchProductIdsBySegmentId($object, $segmentId)
    {
        ...
        if (Mage::helper('sminmlc_customersegment')->isEnabled()) {
     ...
     *
     * @param Sminmlc_TargetRule_Model_Index $object
     * @return Sminmlc_TargetRule_Model_Resource_Index
     */
    ...
    {
        /** @var $targetRule Sminmlc_TargetRule_Model_Resource_Rule */
        $targetRule = Mage::getResourceSingleton('sminmlc_targetrule/rule');
    ...
    protected function _getSegmentsIdsFromCurrentCustomer()
    {
        ...
        if (Mage::helper('sminmlc_customersegment')->isEnabled()) {
            ...
            if (!$customer->getId()) {
                ...
                $segmentIds = Mage::getSingleton('sminmlc_customersegment/customer')
                    ->getCustomerSegmentIdsForWebsite($customer->getId(), $websiteId);
            }

            if (count($segmentIds)) {
                $segmentIds = Mage::getResourceModel('sminmlc_customersegment/segment')
