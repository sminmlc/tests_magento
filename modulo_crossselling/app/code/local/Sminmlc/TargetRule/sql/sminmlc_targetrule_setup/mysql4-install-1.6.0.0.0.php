...
$installer->run("
CREATE TABLE `{$installer->getTable('sminmlc_targetrule/rule')}` (
...
$installer->run("
CREATE TABLE `{$installer->getTable('sminmlc_targetrule/customersegment')}` (
  ...
  CONSTRAINT `FK_ENTERPRISE_TARGETRULE_CUSTOMERSEGMENT_RULE` FOREIGN KEY (`rule_id`)
    REFERENCES `{$installer->getTable('sminmlc_targetrule/rule')}` (`rule_id`)
      ON DELETE CASCADE
      ON UPDATE CASCADE,
  CONSTRAINT `FK_ENTERPRISE_TARGETRULE_CUSTOMERSEGMENT_SEGMENT` FOREIGN KEY (`segment_id`)
    REFERENCES `{$installer->getTable('sminmlc_customersegment/segment')}` (`segment_id`)
      ON DELETE CASCADE
      ON UPDATE CASCADE
) ENGINE=INNODB DEFAULT CHARSET=utf8;
");
...
$installer->run("
CREATE TABLE `{$installer->getTable('sminmlc_targetrule/product')}` (
  `rule_id` INT(10) UNSIGNED NOT NULL,
  ...
  CONSTRAINT `FK_ENTERPRISE_TARGETRULE_PRODUCT_RULE` FOREIGN KEY (`rule_id`)
    REFERENCES `{$installer->getTable('sminmlc_targetrule/rule')}` (`rule_id`) ON DELETE CASCADE ON UPDATE CASCADE
) ENGINE=INNODB DEFAULT CHARSET=utf8;
");

// create target rule frontend index tables
$installer->run("
CREATE TABLE `{$installer->getTable('sminmlc_targetrule/index')}` (
...
CREATE TABLE `{$installer->getTable('sminmlc_targetrule/index_related')}` (
  `entity_id` INT(10) UNSIGNED NOT NULL,
  `store_id` SMALLINT(5) UNSIGNED NOT NULL,
  `customer_group_id` SMALLINT(5) UNSIGNED NOT NULL,
...
CREATE TABLE `{$installer->getTable('sminmlc_targetrule/index_upsell')}` (
  `entity_id` INT(10) UNSIGNED NOT NULL,
  `store_id` SMALLINT(5) UNSIGNED NOT NULL,
  `customer_group_id` SMALLINT(5) UNSIGNED NOT NULL,
  `value` CHAR(255) NOT NULL,
...
CREATE TABLE `{$installer->getTable('sminmlc_targetrule/index_crosssell')}` (
  `entity_id` INT(10) UNSIGNED NOT NULL,
  `store_id` SMALLINT(5) UNSIGNED NOT NULL,
  `customer_group_id` SMALLINT(5) UNSIGNED NOT NULL,
  `value` CHAR(255) NOT NULL,
